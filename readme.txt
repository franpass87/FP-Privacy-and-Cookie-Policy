...
= 1.3.1 =
* Fixed: FooterPolicyLinks — rimosso codice duplicato dopo la classe (parse error / errore critico).
* Added: controllo sintassi PHP (`composer lint:syntax` / GitHub Actions).
...
= 1.2.6 =
* Fixed: modal preferenze — toggle allineati tra categorie (stessa colonna per lo switch indipendentemente dalla lunghezza di Obbligatorio/Abilitato).

= 1.2.5 =
* Changed: banner — titoli tab uniformi (classe `.fp-privacy-banner-panel-title`), tab Info con intestazione visiva; modal preferenze — switch a destra del titolo categoria.

= 1.2.4 =
* Fixed: tema colore Banner — salvataggio esplicito del preset nel dropdown; niente reset visivo dopo «Salva».

= 1.2.3 =
* Changed: admin — niente skip link ad ancora interna, niente scroll automatico al cambio tab.

= 1.2.2 =
* Changed: griglia colori hex personalizzati (Banner) — nuovo layout e CSS da zero, accessibilità migliorata.

= 1.2.1 =
* Fixed: salvataggio palette quando i campi hex non arrivano nel POST — si mantiene la palette già salvata.

= 1.2.0 =
* Added: scheda Banner — selettore tema colore (palette predefinite) + personalizzato opzionale nei dettagli.

= 1.1.16 =
* Fixed: select admin — più spazio a destra per la freccia del dropdown (niente sovrapposizione sul testo).

= 1.1.15 =
* Fixed: scheda Cookie — allineamento checkbox, testo e icona aiuto (granularità EDPB).

= 1.1.14 =
* Fixed: banner pagina Impostazioni — dimensione font titolo come FP Mail (niente override da regole h2 scheda).
...
= 1.1.11 =
* Changed: Impostazioni — rimosso banner breadcrumb + testo guida sopra le tab.
...
* Added: pagina Strumenti — card “Ripristina predefiniti” (con conferma) al posto del reset da Impostazioni.

= 1.1.8 =
* Changed: pagina Impostazioni — rimossa card Azioni rapide; link rapidi sotto l’intro.

= 1.1.7 =
* Fixed: tooltip help sopra icona, z-index e overflow accordion/card; allineamento checkbox e righe nella scheda Privacy e consenso.

= 1.1.6 =
* Changed: admin in italiano (stringhe, descrizioni sezioni, JS); tooltip help con z-index e posizione sotto icona.

= 1.1.5 =
* Changed: modal preferenze — layout più compatto in verticale (padding, card categorie, footer).

= 1.1.4 =
* Added: tab Dettagli banner — lista servizi rilevati dal detector; REST GET `detected-services`; polling opzionale (filtro `fp_privacy_banner_detector_poll_interval_ms`).
