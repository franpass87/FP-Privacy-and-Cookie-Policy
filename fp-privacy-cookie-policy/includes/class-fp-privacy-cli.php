...
class FP_Privacy_CLI {
     ...
     * ## EXAMPLES
     *
     *     wp fp-privacy settings-export --file=fp-settings.json
     *
     ...
     * @param array $assoc_args Associative arguments.
     */
    public function settings_export( $args, $assoc_args ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedParameter
        if ( empty( $assoc_args['file'] ) ) {
            \WP_CLI::error( __( 'You must provide a destination file via the --file parameter.', 'fp-privacy-cookie-policy' ) );
        }
        ...
        $payload = $this->plugin->get_settings_export_payload();
        $json    = wp_json_encode( $payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES );

        if ( false === $json ) {
            \WP_CLI::error( __( 'Unable to encode the settings payload as JSON.', 'fp-privacy-cookie-policy' ) );
        }

        $written = @file_put_contents( $file_path, $json ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_file_put_contents

        if ( false === $written ) {
            \WP_CLI::error( sprintf( __( 'Unable to write settings export to %s.', 'fp-privacy-cookie-policy' ), $file_path ) );
        }

        \WP_CLI::success( sprintf( __( 'Settings exported to %s.', 'fp-privacy-cookie-policy' ), $file_path ) );
    }

    /**
     * Import plugin settings from a JSON file generated by the plugin.
     *
     * ## OPTIONS
     *
     * [--file=<file>]
     * : Path to the JSON file exported from the plugin.
     *
     * ## EXAMPLES
     *
     *     wp fp-privacy settings-import --file=fp-settings.json
     *
     ...
     * @param array $assoc_args Associative arguments.
     */
    public function settings_import( $args, $assoc_args ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedParameter
        if ( empty( $assoc_args['file'] ) ) {
            \WP_CLI::error( __( 'You must provide a destination file via the --file parameter.', 'fp-privacy-cookie-policy' ) );
        }
        ...
        if ( ! file_exists( $file_path ) ) {
            \WP_CLI::error( sprintf( __( 'The file %s could not be found.', 'fp-privacy-cookie-policy' ), $file_path ) );
        }

        if ( ! is_readable( $file_path ) ) {
            \WP_CLI::error( __( 'The provided file could not be read.', 'fp-privacy-cookie-policy' ) );
        }
        ...
        $result = $this->plugin->import_settings_from_payload( $data );

        if ( is_wp_error( $result ) ) {
            \WP_CLI::error( $result->get_error_message() );
        }

        \WP_CLI::success( __( 'Settings imported successfully.', 'fp-privacy-cookie-policy' ) );
    }
