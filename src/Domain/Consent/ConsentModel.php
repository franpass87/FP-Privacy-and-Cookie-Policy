...
class LogModel {
	 ...
	 * Table manager.
	 *
	 * @var ConsentTable|LegacyLogModelTable
	 */
	 ...
	 * Constructor.
	 *
	 * @param ConsentTable|LegacyLogModelTable|null $table_manager Table manager (optional, will be created if not provided).
	...
	public function __construct( $table_manager = null, ?DatabaseInterface $database = null ) {
		...
		if ( $table_manager ) {
			...
			if ( class_exists( '\\FP\\Privacy\\Infrastructure\\Database\\ConsentTable' ) ) {
				$this->table_manager = new ConsentTable( $table_name, $database );
			} else {
				$this->table_manager = new LegacyLogModelTable( $table_name, $database );
			}
		}

		// Use new ConsentTable static methods if available, fallback to old LogModelTable.
		$table_ensured = false;
		if ( class_exists( '\\FP\\Privacy\\Infrastructure\\Database\\ConsentTable' ) ) {
